...
class Pearl(BaseEstimator, TransformerMixin):
    ...
    def get_features_rank(estimator, dataframe: pd.DataFrame, list_of_vars: list, target: str, cv = KFold(), get_fe: bool = False) -> dict:
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(dataframe), 1):
            ...
            #the more sum of feature_importances_ the more rank of it feature (first rank = max(sum feature_importances_))
            dict_fold_importances['fold'] += estimator.feature_importances_
            
            #get dict of estimates as sum feature_importances_ for all folds 
            fe_dict = {key: value for key, value in zip(dict_fold_importances['Feature'], dict_fold_importances['fold'])}
